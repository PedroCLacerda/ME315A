...
3.  Execute as QUERYS a seguir. Examine as tabelas resultantes e faça as observações relevantes.
    ...
    R: As duas primeiras QUERYS não apresental alterações, uma vez que a função DISTINCT elimina os valores iguais entre as linhas, porém já não havia nenhum valor igual de DNO e SALARY. A última QUERY simplesmente ordena os valores de DNO e depois de SALARY de forma crescente.
...
R: Sim, pois segue a regra C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3)
...
R: Sim, pois segue a regra C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3)
...
R: O primeiro e o terceiro são equivalentes, enquanto o segundo não pois não segue a regra NOT (C1 AND C2) = (NOT C1) OR (NOT C2)
...
R: O primeiro e o terceiro são equivalentes, enquanto o segundo não pois não segue a regra NOT (C1 OR C2) = (NOT C1) AND (NOT C2)
